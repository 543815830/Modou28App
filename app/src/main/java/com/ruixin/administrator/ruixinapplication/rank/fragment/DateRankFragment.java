...
import com.ruixin.administrator.ruixinapplication.rank.adapter.RankingListAdapter;
...
public class DateRankFragment extends BaseFragment {
    ...
    protected View initView() {
        if(view==null) {
         ...
         /*   mLoadingAndRetryManager = LoadingAndRetryManager.generate(mListView, new OnLoadingAndRetryListener() {
                @Override
...
//重新加载数据的操作
    ...
    private class MyRankAsyncTask extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... strings) {
            String result= AgentApi.dopost3(URL.getInstance().Rank_URL,prarams);
            ...
            if(s.length()>0){
                ...
                if (index == 0) {
                    ...
                    if (entry.getStatus() == 1) {
                        ...
                        List<RankDb.DataBean.YingliBean> list = rankDb.getData().getYingli();
                        if (list != null && list.size() > 0) {
                            adapter = new RankingListAdapter(mContext, list);
                            mListView.setAdapter(adapter);
                            mLoadingAndRetryManager.showContent();
                        } else {
                            Log.e("list", "first2" + first2);
                           /* if(first2){
