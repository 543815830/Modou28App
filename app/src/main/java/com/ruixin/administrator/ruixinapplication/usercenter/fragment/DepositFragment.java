...
import com.ruixin.administrator.ruixinapplication.usercenter.activiy.GetReliefActivity;
...
public class DepositFragment extends BaseFragment {
    ...
    Button commit;
    String userId="", userSecques="" ,tbAccessType="1",tbUserSecAns="",tbAccessG="",userToken="";
    User user;
    ...
    protected View initView() {
        if(view==null){
            view=View.inflate(mContext, R.layout.fm_deposit,null);
    ...
    private class DepositeAsyncTask extends AsyncTask<String, Integer, String> {

        @Override
        protected String doInBackground(String... strings) {
            String result= AgentApi.dopost3(URL.getInstance().InsideBank_URL,prarams);
            Log.e("存入的数据返回",""+result);
            return result;
        }
                ...
                if (index == 0) {
                    ...
                    if(user.getStatus()==1) {
                       ...
                       Toast.makeText(mContext, user.getMsg(), Toast.LENGTH_SHORT).show();
                        ...
                        /*抗攻击*/
                        Unlogin.doAtk(prarams,s,new DepositeAsyncTask());
                    }else{
                        Toast.makeText(mContext, user.getMsg(), Toast.LENGTH_SHORT).show();
                    }
    ...
    private class MyOnclick implements View.OnClickListener {
        ...
        public void onClick(View view) {
            tbAccessG=et_deposit_coin.getText().toString().trim();
            tbUserSecAns=et_user_secans.getText().toString().trim();
            if(isValid()){
