...
public class WaterRippleView extends View {
    ...
    public WaterRippleView(Context context, AttributeSet attrs) {
        ...
        // 设置画笔颜色
        mWavePaint.setColor(Color.parseColor("#22df9e"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mTotalWidth; i++) {
           ...
           canvas.drawLine(i, 0 , i,
                    600 - mResetOneYPositions[i] , mWavePaint);

            // 绘制第二条水波纹
           /* canvas.drawLine(i, mTotalHeight - mResetTwoYPositions[i] - 400, i,
                    mTotalHeight, mWavePaint);*/
            canvas.drawLine(i, 0, i,
                    400 - mResetOneYPositions[i], mWavePaint);
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // 记录下view的宽高
        mTotalWidth = w;
        Log.e("tag",""+mTotalWidth);
        mTotalHeight = h;
        Log.e("tag",""+mTotalHeight);
        // 用于保存原始波纹的y值
