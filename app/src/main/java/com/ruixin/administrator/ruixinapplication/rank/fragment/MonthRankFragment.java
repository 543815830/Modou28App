...
import com.ruixin.administrator.ruixinapplication.rank.adapter.MRankingListAdapter;
...
public class MonthRankFragment extends BaseFragment {
    ...
    private MRankingListAdapter adapter;
    private View view;
    boolean first=true;
            ...
            if(s.length()>0){
                ...
                if (index == 0) {
                    ...
                    if (entry.getStatus() == 1) {
                        ...
                        List<RankDb.DataBean.YingliWeekBean> list = rankDb.getData().getYingli_week();
                        if (list != null && list.size() > 0) {
                            adapter = new MRankingListAdapter(mContext, list);
